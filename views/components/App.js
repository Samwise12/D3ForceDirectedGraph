...
window.addEventListener('resize', this.componentDidMount.bind(this));		
	...
	componentDidUpdate(prevProps, prevState) {
		...
		d3.selectAll('#flags span').remove();
	...
	buildSVG(data) {
		...
		let force = d3.forceSimulation()
			.force('link', d3.forceLink().distance(18))
    ...
    let flags = objNodes.append('span')
    	.on('mouseover', d=> {
    		this.setState({
    			tooltipDisplay: true,
    			tooltipX: d3.event.clientX-190,
    			tooltipY: d3.event.clientY-13,
    			tooltipHTML: <strong>{d.country}</strong>
    		})
    	})
    	.on('mouseout', ()=>this.setState({ tooltipDisplay: false }))
    	.call(d3.drag()
    		.on('start', (d)=> {
    			if(!d3.event.active) {
    ...
    flags
    	...
    	.attr('style', d=>`left:${d.x-4}px; 
    		top:${d.y-8+this.state.top}px;`)
    	});	    
		...
		return(			
			<div>
				<Tooltip id='tooltip' placement='left' className='in'
					style={{
						display	: this.state.tooltipDisplay ? 'block' : 'none',
						position: 'fixed',
				...
				<h1>Force-Directed Graph Displaying Connected Countries </h1>
				<div id="chart" className='panel panel-default' />			
				<div id="flags" className="f16" style={{position: 'absolute', top: 0}} />
				</div>
		);
	}
}
